...
ABLATIONS = {
    "full_features": [],
    "no_geo": ["latitude", "longitude"],
    "no_geo_no_background": ["latitude", "longitude", "background_rate_yr"],
}
...
def load_dataset(cfg: dict, target: str, drop_features: list[str]):
    ...
    excluded_cols.update(drop_features)
...
def main():
    ...
    for target in cfg["dataset"]["target_columns"]:
        ...
        for variant_name, drop_features in ABLATIONS.items():
            X_train, y_train, X_val, y_val, X_test, y_test, feature_cols = load_dataset(
                cfg, target, drop_features
            )
            ...
            run_name = f"{cfg['benchmark_id']}_ablation_{variant_name}_{horizon}_{git_sha[:7]}"

            with mlflow.start_run(run_name=run_name):
                mlflow.set_tags(
                    {
                        "benchmark_id": cfg["benchmark_id"],
                        "run_kind": "feature_ablation",
                ...
                mlflow.log_params(
                    {
                        ...
                        "dropped_features": ",".join(drop_features) if drop_features else "none",
                        **params,
                    }
                )
                ...
                artifact_dir = PROJECT_ROOT / "experiments" / "reports" / "ablation"
                artifact_dir.mkdir(parents=True, exist_ok=True)

                report_path = artifact_dir / f"{target}_{variant_name}.json"
                ...
                mlflow.log_artifact(str(report_path), artifact_path=f"ablation/{horizon}")

                print(f"Logged ablation run for {target} / {variant_name}")
